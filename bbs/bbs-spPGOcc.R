# bbs-spPGOcc.R: this script runs a single species spatial occupancy model for 
#                Black-throated Blue Warbler across the eastern US in 2018. 
#                The model is fit with a nearest neighbor Gaussian process.
...
out <- spPGOcc(occ.formula = occ.formula,
	       ...
	       n.report = n.report)
# Save results
save(out, file = paste("results/bbs-spPGOcc-", chain, "-", 
		       Sys.Date(), ".R", sep = ''))
