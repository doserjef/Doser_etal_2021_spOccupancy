# bbs-PGOcc-cross-val.R: this script runs a single species occupancy model
#                        with 10-fold cross-validation for Black-throated
#                        Blue Warbler across the eastern US in 2018. 
...
out <- PGOcc(occ.formula = occ.formula,
	     ...
	     k.fold.threads = 10)
# Save results
save(out, file = paste("results/bbs-PGOcc-cross-val-", 
		       Sys.Date(), ".R", sep = ''))
